...
class TTLFileManager:
    ...
    def open_ttl_file(self, input_path: str) -> Tuple[bytes, bool]:
        ...
        # Decode image and convert to bytes
        img = decode_qoi_to_image(qoi_data)
        image_bytes = convert_image_to_bytes(img)
        
        logging.info("Image decrypted to %d bytes (fallback=%s)", len(image_bytes), fallback)
